...
export function SmoothSelect({
  ...
  return (
    <div className={`smooth-select relative grid gap-2 ${open ? "z-[200]" : "z-10"} ${className}`} ref={rootRef}>
      ...
      <div
        ...
        className={`smooth-select-menu absolute left-0 right-0 top-full z-[220] mt-2 max-h-64 overflow-auto rounded-2xl border p-1 shadow-xl backdrop-blur transition-all duration-200 ease-out ${open ? "pointer-events-auto translate-y-0 scale-100 opacity-100" : "pointer-events-none -translate-y-1 scale-[0.98] opacity-0"}`}
